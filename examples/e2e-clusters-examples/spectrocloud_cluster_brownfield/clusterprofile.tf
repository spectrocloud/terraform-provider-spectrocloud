...
# Brownfield profiles skip the OS and CSI layers other cloud types need - Palette does not
# provision the underlying host or storage for an already-existing cluster, so `cloud = "generic"`
# here is just Kubernetes + CNI + one add-on pack, matching cluster_type = "generic" on the
...
resource "spectrocloud_cluster_profile" "brownfield_profile" {
  name        = "e2e-brownfield-full-stack"
  description = "Comprehensive brownfield (generic) cluster profile for the end-to-end usecase example"
  cloud       = "generic"
  type        = "cluster"
  context     = "project"
  ...
  profile_variables {
    ...
    variable {
      ...
      Demonstrates the full spectrocloud_cluster_brownfield attribute surface.
      EOT
    }
  }
}
