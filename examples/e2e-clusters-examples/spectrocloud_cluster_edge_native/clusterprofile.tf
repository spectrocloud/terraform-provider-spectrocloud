...
# Edge Native profiles have no OS pack (edge appliances bring their own image - "BYOI") and
# typically no CSI pack (on-prem storage is usually handled by the OS image / a vendor-specific
# pack rather than a Palette-managed CSI layer) - so this profile is just Kubernetes + CNI + one
# add-on pack.
#
...
resource "spectrocloud_cluster_profile" "edge_native_profile" {
  name        = "e2e-edge-native-full-stack"
  description = "Comprehensive Edge Native cluster profile for the end-to-end usecase example"
  cloud       = "edge-native"
  type        = "cluster"
  context     = "project"
  ...
  profile_variables {
    ...
    variable {
      ...
      Demonstrates the full spectrocloud_cluster_edge_native attribute surface.
      EOT
    }
  }
}
