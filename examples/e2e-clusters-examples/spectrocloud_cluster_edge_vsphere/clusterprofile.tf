...
# Like Edge Native, Edge vSphere profiles skip the OS pack (the edge appliance provisions VM
# images itself via its embedded vSphere-compatible layer) and the CSI pack (on-prem storage is
# handled outside a Palette-managed CSI layer here) - so this profile is just Kubernetes + CNI +
# one add-on pack.
#
...
resource "spectrocloud_cluster_profile" "edge_vsphere_profile" {
  name        = "e2e-edge-vsphere-full-stack"
  description = "Comprehensive Edge vSphere cluster profile for the end-to-end usecase example"
  cloud       = "edge-vsphere"
  type        = "cluster"
  context     = "project"
  ...
  profile_variables {
    ...
    variable {
      ...
      Demonstrates the full spectrocloud_cluster_edge_vsphere attribute surface.
      EOT
    }
  }
}
