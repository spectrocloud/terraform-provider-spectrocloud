# Creates a dedicated backup storage location for this end-to-end example. Edge clusters, like
# other on-prem deployments, commonly back up to a self-hosted, S3-compatible object store
# (Minio) rather than a public cloud bucket.
#
...
resource "spectrocloud_backup_storage_location" "bsl" {
  name             = "e2e-edge-native-backup-location"
  storage_provider = "minio"
  context          = "project"
  is_default       = false

  region      = "minio-default"
  bucket_name = "e2e-edge-native-backups"
