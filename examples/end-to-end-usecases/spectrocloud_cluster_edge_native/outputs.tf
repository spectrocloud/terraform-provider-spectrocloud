...
output "kubeconfig" {
  value     = spectrocloud_cluster_edge_native.cluster.kubeconfig
  sensitive = true
}

# Computed, sensitive. Cluster-admin kubeconfig.
output "admin_kube_config" {
  value     = spectrocloud_cluster_edge_native.cluster.admin_kube_config
  sensitive = true
}

# Computed. Deprecated - the provider notes this field must be of type `azure`, which does not
# apply to Edge Native clusters, so it is expected to remain empty here.
output "cloud_config_id" {
  value = spectrocloud_cluster_edge_native.cluster.cloud_config_id
}

# Computed. Palette-derived geographic location of the cluster - not settable on this resource.
output "location_config" {
  value = spectrocloud_cluster_edge_native.cluster.location_config
}
