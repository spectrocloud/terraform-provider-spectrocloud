# Creates a dedicated backup storage location for this end-to-end example. On-prem clusters like
# MAAS commonly back up to a self-hosted, S3-compatible object store (Minio) rather than a public
# cloud bucket - this demonstrates that path, using the same `s3` block AWS uses but with
# `storage_provider = "minio"` and a custom endpoint URL.
#
...
resource "spectrocloud_backup_storage_location" "bsl" {
  name             = "e2e-maas-backup-location"
  storage_provider = "minio"
  context          = "project"
  is_default       = false

  region      = "minio-default"
  bucket_name = "e2e-maas-backups"
  # Optional CA certificate for SSL connections to a self-signed Minio endpoint.
  # ca_cert = file("${path.module}/minio-ca.pem")
