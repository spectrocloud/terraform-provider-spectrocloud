...
#
# Day-2 note: both spectrocloud_cluster_maas.cluster.kubeconfig and .admin_kube_config are
...
resource "local_file" "kubeconfig" {
  content         = spectrocloud_cluster_maas.cluster.kubeconfig
  filename        = "${var.kubeconfig_output_dir}/kubeconfig_${spectrocloud_cluster_maas.cluster.name}"
...
resource "local_file" "admin_kubeconfig" {
  content         = spectrocloud_cluster_maas.cluster.admin_kube_config
  filename        = "${var.kubeconfig_output_dir}/admin_kubeconfig_${spectrocloud_cluster_maas.cluster.name}"
  file_permission = "0600"
}
