# Creates a dedicated backup storage location for this end-to-end example. A brownfield-imported
# cluster can live on any infrastructure, so this uses a self-hosted, S3-compatible object store
# (Minio) as the most portable choice.
#
...
resource "spectrocloud_backup_storage_location" "bsl" {
  name             = "e2e-brownfield-backup-location"
  storage_provider = "minio"
  context          = "project"
  is_default       = false

  region      = "minio-default"
  bucket_name = "e2e-brownfield-backups"
