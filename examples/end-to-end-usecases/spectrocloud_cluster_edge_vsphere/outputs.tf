output "cluster_id" {
  value = spectrocloud_cluster_edge_vsphere.cluster.id
}

# Computed, sensitive. Also written to disk as a file by kubeconfig.tf - see that file's outputs
# below for the file paths.
output "kubeconfig" {
  value     = spectrocloud_cluster_edge_vsphere.cluster.kubeconfig
  sensitive = true
}

# Computed, sensitive. Cluster-admin kubeconfig.
output "admin_kube_config" {
  value     = spectrocloud_cluster_edge_vsphere.cluster.admin_kube_config
  sensitive = true
}

# Computed. Deprecated in favor of cloud_config, but still populated.
output "cloud_config_id" {
  value = spectrocloud_cluster_edge_vsphere.cluster.cloud_config_id
}

# Computed. Palette-derived geographic location of the cluster.
output "location_config" {
  value = spectrocloud_cluster_edge_vsphere.cluster.location_config
}
