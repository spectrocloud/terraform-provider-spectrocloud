# Comprehensive spectrocloud_cluster_edge_vsphere example - exercises the full attribute surface
# of the resource in one cluster, not just the minimal required fields. See
# examples/resources/spectrocloud_cluster_edge_vsphere for the minimal happy-path version.
#
# Day-2 mutability summary (see inline comments below for the full breakdown):
#   ForceNew (destroy/recreate): name, edge_host_uid, the entire cloud_config block.
#   Updates in place: machine_pool, cluster_profile, backup_policy, scan_policy,
#   cluster_rbac_binding, namespaces, host_config, tags, description, and all the top-level
#   Day-2 operational settings.
#
# Note: unlike the plain (public-cloud) spectrocloud_cluster_vsphere, there is no
# cloud_account_id here - the whole cluster runs on a single paired edge appliance instead
# (edge_host_uid, see edgehost.tf), and machine_pool has no min/max autoscaling (fixed count
# only, like Azure/GKE).
resource "spectrocloud_cluster_edge_vsphere" "cluster" {
  # Required, ForceNew.
  name = "e2e-edge-vsphere-cluster"
  ...
  description = "Comprehensive end-to-end usecase cluster exercising the full Edge vSphere schema."
  ...
  # Required, ForceNew. UID of the paired edge appliance this cluster runs on.
  edge_host_uid = spectrocloud_appliance.edge_host.uid

  cluster_profile {
    id = spectrocloud_cluster_profile.edge_vsphere_profile.id
    # Supplies values for the profile_variables declared on the profile.
    variables = {
      app_version      = "1.2.0"
      environment_tier = "staging"
      notes            = "Deployed by the edge-vsphere end-to-end usecase example."
    }
  ...
  # Required, ForceNew (the entire block). vSphere placement and control-plane networking, on
  # the paired edge appliance.
  cloud_config {
    ...
    # ssh_keys is preferred over the deprecated singular ssh_key field - the two are mutually
    # exclusive (ExactlyOneOf).
    ...
    static_ip = false
    # Optional. Network endpoint type for the control plane address, e.g. "DDNS" - relevant when
    # static_ip = false.
    network_type = "DDNS"
  ...
  machine_pool {
    ...
    # No min/max here - unlike vsphere/aws/eks/aks, Edge vSphere machine pools have a fixed
    # count with no autoscaling.
    count = 2
  ...
  backup_policy {
    prefix             = "e2e-edge-vsphere-backup"
  ...
  # location_config is settable on this resource (country_code/latitude/longitude) - not shown
  # here since Palette otherwise derives it automatically. See outputs.tf for reading it back.
}
