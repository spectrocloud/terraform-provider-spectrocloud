...
resource "spectrocloud_cluster_profile" "aws_profile" {
  name        = "e2e-aws-full-stack"
  description = "Comprehensive AWS cluster profile for the end-to-end usecase example"
  cloud       = "aws"
  type        = "cluster"
  context     = "project"
  ...
  profile_variables {
    ...
    variable {
      ...
      Demonstrates the full spectrocloud_cluster_aws attribute surface.
      EOT
    }
  }
}
