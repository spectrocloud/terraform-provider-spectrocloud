# Creates and manages a dedicated AWS cloud account for this end-to-end example, rather than
# looking up one that already exists - so this folder is fully self-contained.
#
# Day-2 mutability: nothing on this resource is ForceNew - every attribute updates in place.
resource "spectrocloud_cloudaccount_aws" "account" {
  # Required.
  name = "e2e-aws-account"
  # Optional, default "project". Allowed: "project", "tenant".
  context = "project"
  # Optional. Required only for a private cluster endpoint reachable via a Private Cloud
  # Gateway; leave unset for a standard public AWS account.
  # private_cloud_gateway_id = var.private_cloud_gateway_id

  # Required (one of the 3 auth types below). "secret" (static keys, used here), "sts"
  # (assumable role), or "pod-identity" (EKS Pod Identity).
  type = "secret"
  # Preferred sensitive alternative to the deprecated `aws_access_key`. The two are mutually
  # exclusive - set only one.
  aws_secured_access_key = var.aws_access_key
  # aws_access_key       = var.aws_access_key  # deprecated, non-sensitive equivalent
  aws_secret_key = var.aws_secret_key

  # STS alternative (mutually exclusive with the secret-key fields above; set type = "sts"):
  # arn         = var.aws_sts_role_arn
  # external_id = var.aws_sts_external_id

  # EKS Pod Identity alternative (set type = "pod-identity"):
  # role_arn                = var.aws_pod_identity_role_arn
  # permission_boundary_arn = var.aws_permission_boundary_arn

  # Optional, default "aws". Allowed: "aws", "aws-us-gov".
  partition = "aws"
  # Optional. Extra IAM policy ARNs to associate with the account beyond Palette's defaults.
  policy_arns = []
}

# Alternative to creating a new account: look up one that's already registered in Palette.
# data "spectrocloud_cloudaccount_aws" "account" {
#   name = "some-existing-account-name"
# }
