# Creates a dedicated backup storage location for this end-to-end example. A virtual cluster can
# run on top of a host cluster on any infrastructure, so this uses a self-hosted, S3-compatible
# object store (Minio) as the most portable choice.
#
...
resource "spectrocloud_backup_storage_location" "bsl" {
  name             = "e2e-virtual-cluster-backup-location"
  storage_provider = "minio"
  context          = "project"
  is_default       = false

  region      = "minio-default"
  bucket_name = "e2e-virtual-cluster-backups"
