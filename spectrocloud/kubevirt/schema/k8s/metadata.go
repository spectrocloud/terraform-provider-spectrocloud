...
func metadataFields(objectName string) map[string]*schema.Schema {
	return map[string]*schema.Schema{
		"annotations": {
			Type:         schema.TypeMap,
			Description:  fmt.Sprintf("An unstructured key/value map stored with the %s that can be used to store arbitrary metadata. More info: http://kubernetes.io/docs/user-guide/annotations.", objectName),
			Optional:     true,
		...
		"labels": {
			Type:         schema.TypeMap,
			Description:  fmt.Sprintf("Map of string key/value labels used to organize and categorize the %s. May match selectors of replication controllers and services. More info: http://kubernetes.io/docs/user-guide/labels.", objectName),
			Optional:     true,
			Elem:         &schema.Schema{Type: schema.TypeString},
		...
		"name": {
			...
			Description:  fmt.Sprintf("Name of the %s. Must be unique and cannot be updated. More info: http://kubernetes.io/docs/user-guide/identifiers#names.", objectName),
			Optional:     true,
			Computed:     true,
			ForceNew:     true,
