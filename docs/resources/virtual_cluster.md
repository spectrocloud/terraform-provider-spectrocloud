...
- `context` (String) The context of the virtual cluster. Can be `project` or `tenant`. Default is `project`.If `project` context is specified, `project_name` will be taken from provider configuration. Default value for `project_name` is `Default`.
