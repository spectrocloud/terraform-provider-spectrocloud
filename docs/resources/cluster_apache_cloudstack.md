...
resource "spectrocloud_cluster_apache_cloudstack" "advanced_cluster" {
  ...
  machine_pool {
    ...
    # Optional: Rolling update strategy for controlled updates
    rolling_update_strategy {
      type            = "OverrideScaling"  # RollingUpdateScaleOut, RollingUpdateScaleIn, or OverrideScaling
...
resource "spectrocloud_cluster_apache_cloudstack" "cluster_from_template" {
  ...
  machine_pool {
    ...
    # Optional: Rolling update strategy
    rolling_update_strategy {
...
### Machine Pool Update Strategy Options

The `rolling_update_strategy` block allows fine-grained control over how nodes are updated during cluster upgrades:

- **RollingUpdateScaleOut**: Adds new nodes before removing old ones (safest, requires extra capacity)
- **RollingUpdateScaleIn**: Removes old nodes before adding new ones (more resource-efficient)
- **OverrideScaling**: Provides precise control with `max_surge` and `max_unavailable` parameters

```terraform
# Example 1: Safe updates with extra capacity
rolling_update_strategy {
  type            = "RollingUpdateScaleOut"
  max_surge       = "25%"  # Add 25% extra nodes during update
  max_unavailable = "0"    # Never have unavailable nodes
}

# Example 2: Resource-efficient updates
rolling_update_strategy {
  type            = "RollingUpdateScaleIn"
  max_surge       = "0"    # Don't add extra nodes
  max_unavailable = "1"    # Allow one node to be unavailable at a time
}

# Example 3: Balanced approach
rolling_update_strategy {
  type            = "OverrideScaling"
  max_surge       = "1"    # Add one extra node
  max_unavailable = "1"    # Allow one node to be unavailable
}
```

**Note**: The deprecated `update_strategy` field is still supported for backward compatibility but will be removed in a future version. Use `rolling_update_strategy` instead.
...
- `update_strategy` (String) Update strategy for the machine pool. Valid values are `RollingUpdateScaleOut` and `RollingUpdateScaleIn`. This field is deprecated. Use `rolling_update_strategy` instead. If both are specified, `rolling_update_strategy` takes precedence.
